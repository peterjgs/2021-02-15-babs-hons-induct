...
Sample size calculator: https://vbiostatps.app.vumc.org/ps/
...
pwr.all %>% 
  ...
  geom_line(size = 1) +
  labs(x = "Group sample size", y = "Power", colour = "Diff (sd)") +
...
- Biological replication (this is what "sample size" means) is what counts towards power.
...
> - Variance ( $= (\mathrm{std. dev.})^2$) measures variation, which introduces uncertainty into estimates
...
- `r colourise("P value is an indication of *how unusual* a sample is, assuming the null hypothesis is true (in this case, that the two groups have the same mean).", "#8000ff")`
...
`r colourise("The P value is the probability of getting the data we got, *assuming the null hypothesis is true*.", "#8000ff")`
